"""
core/audio_devices.py — pick which microphone and which speakers Assistant uses.
...
WHY
    ...
    "Assistant can't hear me" almost always means "Assistant is listening to the
...
WHY THIS IS CACHED
    ...
    milliseconds on a Windows machine with many endpoints. Smart Assistant learned this
...
def switch_output_device(device_name: str) -> dict[str, Any]:
    """Switch the audio output endpoint or truthfully report Windows switching policy."""
    cleaned = (device_name or "").strip()
    if not cleaned:
        return {"success": False, "message": "Please specify an audio output device name."}

    outputs = list_devices("output")
    if not outputs:
        return {"success": False, "message": "No active audio output devices found on this system."}

    matched = None
    target_low = cleaned.lower()
    for dev in outputs:
        dev_low = dev.lower()
        if target_low == dev_low or target_low in dev_low or dev_low in target_low:
            matched = dev
            break

    if not matched:
        resolved_idx = resolve(cleaned, "output")
    ...
    if not matched:
        ...
        return {
            ...
            "matched": False,
            "device": cleaned,
            "message": f"Audio output device '{cleaned}' was not found. Available outputs: {out_str}."
        }
    ...
    return {
        ...
        "matched": True,
        "device": matched,
        "message": f"I found the device '{matched}', but changing the Windows default output is not currently supported on this system without elevated administrative configuration."
    }
