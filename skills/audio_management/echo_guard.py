...
class EchoGuard:
    ...
    def process_frame(self, chunk: np.ndarray, sample_rate: int = 16000) -> bool:
        """Process an incoming microphone frame.
        
        Returns:
            True if the frame is genuine user speech.
            False if the frame is acoustic echo from assistant playback or silence.
        Gracefully returns True on any parsing or arithmetic errors to avoid dropping speech.
        """
        try:
            if chunk is None or len(chunk) == 0:
                return False
            # Normalize level to 0.0 - 1.0 (assuming int16 format)
            rms = float(np.sqrt(np.mean(chunk.astype(np.float32) ** 2)))
            level = min(1.0, max(0.0, rms / 32768.0))
            return self.is_user_speech(chunk, sample_rate, level)
        except Exception:
            return True


_global_echo_guard: EchoGuard | None = None


def get_echo_guard() -> EchoGuard:
    """Singleton instance of EchoGuard for shared microphone/playback tracking."""
    global _global_echo_guard
    if _global_echo_guard is None:
        _global_echo_guard = EchoGuard()
    return _global_echo_guard
