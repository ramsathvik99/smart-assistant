...
_global_ptt: PushToTalk | None = None


def _default_ptt_callback(held: bool) -> None:
    try:
        from legacy.sst import set_ptt_held
        set_ptt_held(held)
    except Exception:
        pass


def get_push_to_talk(on_change: Callable[[bool], None] | None = None, chord=DEFAULT_CHORD) -> PushToTalk:
    """Retrieve or initialize singleton PushToTalk listener."""
    global _global_ptt
    if _global_ptt is None:
        cb = on_change if on_change is not None else _default_ptt_callback
        _global_ptt = PushToTalk(on_change=cb, chord=chord)
    return _global_ptt


def start_push_to_talk() -> str:
    """Begin polling for the push-to-talk chord without altering continuous listening."""
    ptt = get_push_to_talk()
    return ptt.start()


def stop_push_to_talk() -> None:
    """Cleanly stop push-to-talk watcher thread."""
    global _global_ptt
    if _global_ptt is not None:
        _global_ptt.stop()
...
def is_ptt_enabled() -> bool:
    global _ptt_enabled
    return _ptt_enabled


def set_ptt_enabled(enabled: bool) -> str:
    global _ptt_enabled
    _ptt_enabled = bool(enabled)
    if _ptt_enabled:
        scope = start_push_to_talk()
        chord = chord_label()
        return f"Push to talk enabled ({chord}, scope: {scope})."
    else:
        stop_push_to_talk()
        return "Push to talk disabled."


def toggle_ptt() -> str:
    global _ptt_enabled
    return set_ptt_enabled(not _ptt_enabled)
